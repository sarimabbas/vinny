...
A new server listens without authentication on `127.0.0.1`. Users can choose another IPv4 or IPv6 address, enable VeNCrypt/X509Plain with a password, allow legacy VNC authentication for macOS Screen Sharing, or disable remote control. Passwords are stored in the macOS Keychain.
...
- an eight-byte password limit and plaintext warning for optional legacy authentication
...
- `vendor/rustvncserver/src/client.rs`: RFB parsing, legacy VNC authentication, VeNCrypt, and TLS authentication
