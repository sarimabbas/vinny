...
7. Future code enables vendored VNC password auth → a client selects an unoffered/invalid security type → current negotiation logic may bypass the intended security choice.
...
| TM-008 | Malformed VNC client | Future password authentication is enabled | Select a security type not offered by the server | Authentication bypass in a future feature | Input/screen access | Vinny currently always passes `None` (`src/main.rs`) | Vendored handshake does not strictly reject every unoffered security choice | Before enabling passwords, require selected type to equal the offered type and add negative negotiation tests; prefer modern auth over legacy DES | Authentication-failure and invalid-negotiation logs | Not applicable today; medium if enabled unchanged | High | Medium |
