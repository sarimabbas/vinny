...
- VNC authentication and transport encryption are intentionally absent in the current version (`README.md`; `src/main.rs`, `VncServer::new(..., None)`). Direct public-Internet exposure is unsupported, not technically prohibited.
...
- Only the latest Vinny release receives security fixes (`SECURITY.md`).

Open questions that could change risk: whether future built-in authentication will be modern and encrypted rather than legacy VNC DES, and whether the tap will eventually require fully automatic merges rather than a human-reviewed cask pull request.
...
- **RFB listeners:** Tokio listeners parse unauthenticated client messages and serve framebuffers (`src/main.rs`, `serve`; `vendor/rustvncserver/src/server.rs`, `listen_on`; `vendor/rustvncserver/src/client.rs`, `handle_messages`).
...
- **VNC client → Vinny listener:** RFB messages, keyboard events, pointer events, and encoding requests cross TCP. There is no authentication, encryption, client allowlist, connection limit, or rate limit. Protocol fields and clipboard size receive some validation in `vendor/rustvncserver/src/client.rs`.
- **Vinny → VNC client:** captured screen pixels cross the same plaintext TCP channel. Any connected client receives sensitive screen content.
...
|---|---|---|---|---|
| RFB TCP listener | Configured IP and port | Network → privileged app | No auth/TLS; accepts multiple clients | `src/main.rs`, `serve`; `vendor/rustvncserver/src/server.rs`, `listen_on` |
| RFB message parser | Client byte stream | Untrusted protocol → parser/state | Clipboard capped at 10 MiB; no connection/read timeout | `vendor/rustvncserver/src/client.rs`, `handle_messages` |
...
1. An operator binds Vinny publicly → attacker discovers the port → connects without credentials → reads the screen and sends input as the logged-in user.
2. A network observer reaches plaintext RFB traffic → captures screen contents and keystrokes → recovers sensitive information without interacting with Vinny.
3. An attacker opens many connections or stalls handshakes → each connection creates async state/tasks without timeout or a global cap → Vinny exhausts memory/file descriptors and stops serving legitimate clients.
...
7. Future code enables legacy VNC password authentication → users treat the listener as Internet-safe → attackers target its short DES-based password scheme and plaintext transport.

## Threat model table

| Threat ID | Threat source | Prerequisites | Threat action | Impact | Impacted assets | Existing controls (evidence) | Gaps | Recommended mitigations | Detection ideas | Likelihood | Impact severity | Priority |
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| TM-001 | Remote client | Listener is reachable | Connect without authentication and view/control the Mac | Session compromise and sensitive-data disclosure | Screen, input, user session | Loopback default and warnings (`src/VinnyUI.swift`; `README.md`; `SECURITY.md`) | No authentication, encryption, or allowlist | Keep direct Internet exposure unsupported; add modern authenticated encryption before presenting public binding as safe | Log remote address and connection lifecycle; expose active-client count | High when reachable; low on loopback | High | High |
| TM-002 | On-path network attacker | Attacker can observe a non-tunneled route | Capture or modify plaintext RFB | Screen/keystroke disclosure and input manipulation | Screen, input | Tailscale/trusted-network guidance (`SECURITY.md`) | RFB transport is plaintext | Recommend an authenticated encrypted tunnel; eventually add TLS or a secure protocol layer | Document tunnel verification; log listener interface | Medium on shared networks | High | High |
| TM-003 | Remote client | Listener is reachable | Flood or stall connections and parser state | Resource exhaustion and loss of remote access | Availability | Rust memory safety; 10 MiB clipboard cap (`vendor/rustvncserver/src/client.rs`) | No handshake/read timeout, client cap, or rate limit; completed task handles accumulate | Add handshake/idle timeouts and a conservative concurrent-client cap; prune finished handles | Connection counts, task counts, memory, file-descriptor pressure | High when public | Medium | High |
...
| TM-008 | Remote client | Future legacy VNC password authentication is enabled | Attack the short DES-based secret or plaintext session | False assurance and remote session compromise | Input/screen access | Vinny always passes `None`; unoffered security choices are rejected and regression-tested (`src/main.rs`; `vendor/rustvncserver/src/client.rs`) | Legacy VNC authentication would still provide neither modern password security nor transport encryption | Do not expose the vendored legacy authentication option in Vinny; design modern authenticated encryption before advertising authentication | Authentication-failure and invalid-negotiation logs | Not applicable today; medium if enabled | High | Medium |
...
| `vendor/rustvncserver/src/server.rs` | Accepts unbounded network connections and owns client-task lifecycle | TM-003 |
| `src/input.rs` | Converts remote messages into privileged local input | TM-001, TM-002 |
| `src/main.rs` | Chooses no authentication and connects capture/input/listener lifecycles | TM-001, TM-002, TM-003, TM-008 |
